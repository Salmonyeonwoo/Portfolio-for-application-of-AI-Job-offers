        ...
        if st.button("자료 분석 시작 (RAG Indexing)", key="start_analysis"):
            with st.spinner("자료 분석 및 DB 구축 중..."):
                try:
                    ...
                    st.success(f"총 {len(text_chunks)}개 청크로 DB 구축 완료!")
                except Exception as e:
                    st.error(f"RAG 구축 오류: {e}")
                    st.session_state.is_rag_ready = False
    else:
        st.session_state.is_rag_ready = False
        st.warning("학습 자료 업로드 후 '자료 분석 시작' 버튼 클릭 필요")
...
if feature_selection == "RAG 지식 챗봇":
    ...
    if st.session_state.is_rag_ready:
            ...
            with st.chat_message("assistant"):
                with st.spinner("답변 생성 중..."):
                    try:
                        ...
                        answer = response.get('answer', '응답 생성 불가')
                        st.markdown(answer)
        ...
        st.info("사이드바에서 자료 업로드 후 분석을 시작하세요.")

elif feature_selection == "맞춤형 학습 콘텐츠 생성":
    st.header("맞춤형 학습 콘텐츠 생성")
    st.markdown("주제, 난이도, 형식을 입력하여 AI가 콘텐츠 생성")
    ...
    if st.session_state.is_llm_ready:
        ...
        if st.button("콘텐츠 생성"):
            if topic:
                system_prompt = f"당신은 {level} 수준의 AI 코치입니다. 요청 주제에 대해 {content_type} 생성. 한국어로 답변."
                user_prompt = f"주제: {topic}. 형식: {content_type}."
                with st.spinner(f"{topic} 생성 중..."):
                    try:
                        response = st.session_state.llm.invoke(user_prompt, system_instruction=system_prompt)
                        st.success(f"**{topic}** 결과:")
    ...
    try:
        ...
        if avg_predict > avg_recent:
            comment = "최근 학습 데이터와 예측 결과에 따르면, 앞으로 성취도가 긍정적으로 향상될 것으로 예측됩니다. 현재 학습 방식을 유지하거나 난이도를 올려 도전해 보세요!"
        elif avg_predict < avg_recent - 5:
            comment = "예측 결과, 성취도가 다소 하락할 수 있습니다. 학습 방법이나 자료를 다시 점검하고 RAG 챗봇을 활용해 기초 개념을 확인해 보세요."
        else:
            comment = "성취도는 현재 수준을 유지할 것으로 예측됩니다. 새로운 학습 콘텐츠(예: 실습 아이디어)를 활용하여 학습에 활력을 더하세요."
