# ================================
# Streamlit AI 학습 코치 (최종)
# NLTK punkt_tab 자동 다운로드 포함
# ================================
...
if feature_selection == "RAG 지식 챗봇":
    ...
    st.markdown("가상의 과거 퀴즈 점수 데이터를 바탕으로 LSTM 모델을 훈련하고 미래 성취도를 예측합니다.")
    ...
    with st.spinner("LSTM 모델 로드 및 학습 중..."):
        try:
            ...
            for _ in range(5):
                ...
                # 다음 입력 시퀀스 갱신
                current_input = np.append(current_input[:, 1:, :], [[next_score[0]]], axis=1).reshape(1, look_back, 1)
            ...
            if avg_predict > avg_recent:
                comment = "최근 학습 데이터와 LSTM 예측 결과에 따르면, **앞으로의 학습 성취도가 긍정적으로 향상될 것으로 예측**됩니다. 현재 학습 방식을 유지하거나 난이도를 높여 도전해 보세요!"
            elif avg_predict < avg_recent - 5:
                comment = "LSTM 예측 결과, **성취도가 다소 하락할 수 있다는 신호**가 보입니다. 학습 방법을 점검하고, RAG 챗봇으로 기초 개념을 다시 확인해 보세요."
            else:
                comment = "성취도는 현재 수준을 유지할 것으로 예측됩니다. 새로운 학습 콘텐츠 형식(예: 실습 예제 아이디어)을 생성하여 학습에 활력을 더해보세요."
